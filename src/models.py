...
def Encoder_resnet(x, is_training=True, weight_decay=0.001, reuse=False, resnet_v2=None):
    """
    Resnet v2-50
    ...
    variables = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='resnet_v2_50')
...
def Encoder_fc3_dropout(x,
    ...
    variables = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=scope.name)
    return net, variables
